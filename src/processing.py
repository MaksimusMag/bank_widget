...
def filter_by_state(transactions: List[Dict[str, str]], state: str = "EXECUTED") -> List[Dict[str, str]]:
    ...
    Возвращает:
        List[Dict[str, str]]: Новый список словарей, содержащий только те транзакции,
...
def sort_by_date(transactions: List[Dict[str, str]], ascending: bool = False) -> List[Dict[str, str]]:
    """
    ...
    Аргументы:
        ...
        ascending (bool): Порядок сортировки. False - убывание (сначала новые),
                          True - возрастание (сначала старые). По умолчанию False.
    ...
    Примеры:
        ...
        [{'id': 41428829, ...}, {'id': 594226727, ...}, {'id': 939719570, ...}]

        >>> sort_by_date(transactions, ascending=True)  # возрастание
        [{'id': 939719570, ...}, {'id': 594226727, ...}, {'id': 41428829, ...}]
    """
    if not transactions:
        return []
