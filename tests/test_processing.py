...
class TestFilterByState:
    ...
    def test_filter_by_state_with_missing_state(self) -> None:
...
class TestSortByDate:
    ...
    def test_sort_by_date_same_dates(self, transactions_with_same_dates) -> None:
    ...
    def test_sort_by_date_missing_dates(self, transactions_with_missing_dates) -> None:
    ...
    def test_sort_by_date_ascending_missing_dates(self, transactions_with_missing_dates) -> None:
        """Тестирует сортировку по возрастанию при отсутствии дат."""
        result = sort_by_date(transactions_with_missing_dates, ascending_order=True)
        assert len(result) == 3
        expected_ids = [2, 3, 1]
        result_ids = [item["id"] for item in result]
        assert result_ids == expected_ids

    def test_sort_by_date_invalid_dates(self, invalid_date_transactions) -> None:
    ...
    def test_sort_by_date_various_orders(
        ...
        result = sort_by_date(sample_transactions_simple, ascending_order=ascending)
        result_ids = [item["id"] for item in result]
        assert result_ids == expected_ids
