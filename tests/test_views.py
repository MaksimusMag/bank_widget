...
class TestAggregateExpenses:
    """Тесты для функции aggregate_expenses."""

    def test_aggregate_expenses_valid(self) -> None:
        """Тестирует агрегацию расходов."""
        data = {
            "Категория": ["Супермаркеты", "Супермаркеты", "Транспорт", "Переводы", "Наличные", "Еда"],
            "Сумма платежа": [-100, -200, -50, -500, -300, -30],
        }
        df = pd.DataFrame(data)
        result = aggregate_expenses(df)

        assert result["total_amount"] == 1180
        # 3 основные категории (все, что не Переводы и не Наличные)
        assert len(result["main"]) == 3
        # Переводы и Наличные
        assert len(result["transfers_and_cash"]) == 2

        # Проверяем суммы по категориям
        main_categories = {item["category"]: item["amount"] for item in result["main"]}
        assert main_categories["Супермаркеты"] == 300
        assert main_categories["Транспорт"] == 50
        assert main_categories["Еда"] == 30

        transfers = {item["category"]: item["amount"] for item in result["transfers_and_cash"]}
        assert transfers["Переводы"] == 500
        assert transfers["Наличные"] == 300

    def test_aggregate_expenses_with_other(self) -> None:
        """Тестирует агрегацию расходов с категорией 'Остальное'."""
        # Создаем 10 категорий, чтобы проверить "Остальное"
        data = {
            "Категория": [f"Категория_{i}" for i in range(10)],
            "Сумма платежа": [-100 - i * 10 for i in range(10)],
        }
        df = pd.DataFrame(data)
        result = aggregate_expenses(df)
    ...
    def test_aggregate_expenses_empty(self) -> None:
        """Тестирует агрегацию расходов с пустым датафреймом."""
        df = pd.DataFrame()
        result = aggregate_expenses(df)
...
class TestAggregateIncome:
    """Тесты для функции aggregate_income."""

    def test_aggregate_income_valid(self) -> None:
        data = {
            "Категория": ["Зарплата", "Кешбэк", "Зарплата"],
            "Сумма платежа": [50000, 100, 30000],
        }
        df = pd.DataFrame(data)
        result = aggregate_income(df)

        assert result["total_amount"] == 80100
        assert len(result["main"]) == 2

    def test_aggregate_income_empty(self) -> None:
        df = pd.DataFrame()
        result = aggregate_income(df)
        assert result["total_amount"] == 0
        assert result["main"] == []
...
class TestEventsPage:
    ...
    def test_events_page_valid(self, mock_settings, mock_read_excel) -> None:
...
class TestMainPage:
    ...
    @patch("src.views.pd.read_excel")
    @patch("src.views.load_user_settings")
    def test_main_page_valid(self, mock_settings, mock_read_excel) -> None:
        mock_settings.return_value = {"user_currencies": ["USD"], "user_stocks": ["AAPL"]}
