...
class TestGetDateExceptions:
    """Тесты для обработки исключений в get_date."""

    def test_get_date_with_empty_string(self) -> None:
        """Тестирует обработку пустой строки."""
        assert get_date("") == ""

    def test_get_date_with_whitespace_only(self) -> None:
        """Тестирует обработку строки, состоящей только из пробелов."""
        assert get_date("   ") == "   "

    def test_get_date_with_invalid_format(self) -> None:
        """Тестирует обработку некорректного формата даты."""
        assert get_date("invalid-date") == "invalid-date"

    def test_get_date_with_wrong_separator(self) -> None:
        """Тестирует обработку даты с неправильным разделителем."""
        assert get_date("2024/03/11T02:26:18") == "2024/03/11T02:26:18"

    def test_get_date_without_time(self) -> None:
        """Тестирует обработку даты без времени."""
        assert get_date("2024-03-11") == "11.03.2024"

    def test_get_date_with_incomplete_date(self) -> None:
        """Тестирует обработку неполной даты."""
